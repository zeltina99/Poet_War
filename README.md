...
---
## 🧭 탐험 메뉴 (ExploreMenu)
플레이어는 탐험 중 다음 네 가지 행동을 선택할 수 있습니다.

1. **대화하기** – 윤동주에게 첨삭을 부탁하는 기능  
2. **이동하기** – 새로운 마을이나 지역으로 이동  
3. **조사하기** – 주변을 탐색하며 적 시인을 만남  
4. **가방 열기** – 플레이어의 상태, 시집(아이템), 잉크(골드)를 확인  

---

## ⚔️ 전투 메뉴 (BattleMenu)
전투에 돌입하면 두 가지 행동을 선택할 수 있습니다.

1. **시를 읽는다** – 플레이어가 직접 쓴 시를 낭송 → 기본 공격  
2. **시집을 펼친다** – 획득한 시집의 힘을 사용 → 특수 스킬 발동  
...
| 클래스 | 역할 |
|--------|------|
| **GameManager** | 게임 전체 진행을 관리하는 메인 컨트롤러. 시작, 인트로, 탐험, 최종 보스전까지 전반적인 흐름을 담당. |
| **Battles** | 플레이어와 적(시인) 간의 전투 로직을 관리. 전투 메뉴, 공격/스킬 사용, 보스 페이즈 전환 등이 구현됨. |
| **Chapters** | 스토리 챕터와 탐험 과정을 관리. 다음 적을 만날 때까지의 이벤트, 마을 선택, 스토리 분기 등을 담당. |
| **Player** | 플레이어 데이터 관리. 이름, 시 구절, 체력, 잉크(골드), 인벤토리(시집), 공격력 계산 기능을 포함. |
| **Poet** | 적 시인들의 기본 클래스. 체력, 공격력, 대사, 전투 패턴을 정의. *(보들레르, 단테, 괴테 등은 파생 클래스)* |
| **Map** | 게임 내 지역(마을, 성채 등)을 출력하고, 이동 경로를 관리. |
...
최종보스의 연출에 사용하여, 살아 움직이는 듯한 분위기를 줍니다.  
